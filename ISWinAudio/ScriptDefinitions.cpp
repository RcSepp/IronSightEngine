...
BOOST_PYTHON_MODULE(WinAudio)
{
	...
	class_<Sound, boost::noncopyable>("Sound", no_init)
		.def_readonly("datalen", &Sound::datalen)
		.add_property("format", make_getter(&Sound::format, POLICY_MANAGED_BY_C))
		.def("Play", &Sound::Play)
	;
	objects::register_conversion<IWadSound, Sound>();
	objects::register_conversion<Sound, IWadSound>();

	class_<OutDevice, boost::noncopyable>("OutDevice", no_init)
		.def("CreateSound", &OutDevice::CreateSoundFromFileWrapper, POLICY_MANAGED_BY_C)
		.add_property("volume", &OutDevice::GetVolume, &OutDevice::SetVolume)
		.add_property("leftvolume", &OutDevice::GetLVolume, &OutDevice::SetLVolume)
		.add_property("rightvolume", &OutDevice::GetRVolume, &OutDevice::SetRVolume)
		.def("Pause", &OutDevice::Pause)
		.def("Resume", &OutDevice::Resume)
		.def("Release", &OutDevice::Release)
	;

	class_<WinAudio, boost::noncopyable>("WinAudio")
		.def("Init", &WinAudio::Init)
		/*.def("GetSupportedOutputFormats", &WinAudio::GetSupportedOutputFormatsWrapper, POLICY_MANAGED_BY_PYTHON)*/
		.def("QueryOutputFormat", &WinAudio::QueryOutputFormat)
		.def("OpenOutputDevice", &WinAudio::OpenOutputDeviceWrapper, POLICY_MANAGED_BY_C)
		.def("QueryInputFormat", &WinAudio::QueryInputFormat)
		.def("OpenInputDevice", &WinAudio::OpenInputDeviceWrapper, POLICY_MANAGED_BY_C)
	;
}

WINAUDIO_EXTERN_FUNC void __cdecl RegisterScriptableWinAudioClasses(const IPythonScriptEngine* pse)
{
	if(pse)
		pse->AddModule("WinAudio", &PyInit_WinAudio);
}
void SyncWithPython()
{
	AddToBuiltins("wad", (WinAudio*)&*wad);
}
