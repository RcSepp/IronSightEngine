...
if(dip->mouse.rgbButtons[2])
{
	this->rot.y -= dip->mouse.lX * 0.01f;
	if(this->rot.y > 2.0f * PI)
		this->rot.y -= 2.0f * PI;
	if(this->rot.y < -2.0f * PI)
		this->rot.y += 2.0f * PI;

	this->rot.x = min(PI / 2.0f, max(-PI / 2.0f, this->rot.x - dip->mouse.lY * 0.01f));
} //EDIT11 (DELETE)*/
							  ...
							  const Vector3* center, const Vector3* radius)
{
	...
	Vector3 vBoundCenterWorld, vPlaneNormalWorld;
	...
	for(byte i = 0; i < 6; i++)
	{
		Vec3TransformNormal(&vPlaneNormalWorld, &Vector3(vfrustum[i].a, vfrustum[i].b, vfrustum[i].c), &mScaleRot);
